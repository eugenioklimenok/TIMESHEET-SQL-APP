...
| 2 | Corregir inconsistencias de identificadores y unicidad en cuentas/proyectos/usuarios (campos `account_id`/`project_id` vs `code`) y validar duplicados a nivel de modelo y CRUD. | Alta | 1 | Datos normalizados sin colisiones y endpoints coherentes con el modelo SQL. | Corrección de inconsistencias detectadas | **COMPLETADA** |
| 3 | Reestructurar routers de usuarios/cuentas/proyectos para delegar reglas en una capa de servicios y reutilizar validaciones comunes. | Alta | 1, 2 | Lógica desacoplada, menor duplicidad y rutas listas para expandir reglas de negocio. | Reestructuración necesaria | **COMPLETADA** |

**Resumen técnico Tarea 3**
- Creación de la capa `/services` para users/accounts/projects
- Extracción de validaciones comunes y reglas repetidas desde routers
- Simplificación de routers delegando lógica de negocio a servicios
- Unificación de comportamientos entre CRUD y routers
- Respeto de identificadores/unicidad definidos en Tareas 1 y 2
...
Resumen Fase 1 – Tarea 2 (Completada): Se consolidaron los identificadores de cuentas/proyectos/usuarios manteniendo `id` (UUID) como clave interna y un único código legible por entidad, normalizando los schemas/modelos y agregando validación de unicidad en CRUD y routers para evitar colisiones.
